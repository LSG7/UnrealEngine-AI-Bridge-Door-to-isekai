...
- 누구나 사용가능하도록 하기 위해서 로컬 실행으로 만들 수 밖에 없었다. 서버에서 로직을 돌리면 광고만으로 서비스가 불가능하다.  그런데 로컬로 돌리면 리버스 엔지니어링 공격에 해킹당할 수 밖에 없다. 방어책도 구현해놓긴 했지만 작동 로직이 뚫리면 복제본은 금방 만들어질 수 밖에 없다. 내 복제본이 나타나서 자기가 오리지날이라고 우기면 어떻게 할 수가 없다.   
